...
- Заголовок **hh-user-agent** — задокументирован в OpenAPI-схеме, но на практике сервер его игнорирует (не требует, 200 и без него). Это расхождение между документацией и реальностью; в тестах используется для соответствия примерам из доки, но его отсутствие не блокирует запрос.
